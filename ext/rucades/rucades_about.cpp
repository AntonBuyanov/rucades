...
{
  // Создаем boost::shared_ptr и преобразуем его в std::shared_ptr
  boost::shared_ptr<CryptoPro::PKI::CAdES::CPPVersionObject> boost_p_version(new CryptoPro::PKI::CAdES::CPPVersionObject());
  std::shared_ptr<CryptoPro::PKI::CAdES::CPPVersionObject> std_p_version(boost_p_version);

  hr_method_check(m_pCppCadesImpl->get_PluginVersion(std_p_version));
  return pre_rb_Version(std_p_version);
}
...
pre_rb_Version pre_rb_About::csp_version(std::string prov_name, long prov_type)
{
  ...
  hr_method_check(m_pCppCadesImpl->get_CSPVersion(provName, prov_type, std_p_version));
  return pre_rb_Version(std_p_version);
}
